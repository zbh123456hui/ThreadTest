...
void BlockingQueue::enqueue(user_t user){
...
void BlockingQueue::enitem(user_t user){
...
user_t BlockingQueue::deitem(){
    std::unique_lock<std::mutex> lock(queueMtx);
    user_t user = m_queue.front();
    m_queue.pop_front();
    m_num--;
    //deId++;
    lock.unlock();
